...
public class AddDriver extends JFrame implements ActionListener{
    ...
    JComboBox  availablecombo, cleancombo,agecombo, gendercombo;
    ...
    AddDriver(){
        ...
        String driverOptions[]={"Available", "Taken"};
      ...
      cancel = new JButton("Cancel");
        ...
        cancel.setBackground(new Color(139, 69, 19));
        cancel.setBounds(220,370,130,30);
        cancel.addActionListener(this);
        add(cancel);
        
        
        ImageIcon i1 = new ImageIcon(ClassLoader.getSystemResource("icons/hm.jpg"));
        JLabel image = new JLabel(i1);
        image.setBounds(400, 70, 520, 300);
        add(image);
        
        
        
        
       setBounds(300,200,980,470);// 300,980 to make big
       setVisible(true);
    }
...
public void actionPerformed(ActionEvent ae) {
    if (ae.getSource() == add) {
        ...
        if(location.equals("")){
            JOptionPane.showMessageDialog(null, "Location is a must. Please add.");
            return;
        }
        
        try {
            Conn conn = new Conn();
            String str = "insert into driver (name, age, gender, company, model, available, location) values ('" + name + "', '" + age + "','" + gender + "','" + company + "','" + model + "', '" + available + "', '" + location + "')";
            System.out.println(str); //imp this one 
            conn.s.executeUpdate(str);
            JOptionPane.showMessageDialog(null, "New Driver Has Been Added");
            setVisible(false);
        } catch (Exception e) {
            e.printStackTrace();
            JOptionPane.showMessageDialog(null, "Error: Could not add driver to database");
        }
